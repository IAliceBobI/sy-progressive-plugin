...
function loadStore(plugin: BaseTomatoPlugin) {
    ...
    // 比旧实况多显一级=脑图骨架意图）；坏值归一 headings。.set 只写内存幂等
    // （cssRefEffect 迁移同款纪律）——任意设置保存搭车落盘
    {
        const lv = graphDefaultExpandLevel.get();
        if (!["1", "2", "3", "4", "5", "6", "all", "headings"].includes(lv)) {
            graphDefaultExpandLevel.set("headings");
        }
    }
