...
这里有三种拆分方式，根据标题、内容块数量等分N份、根据字数，可以同使用，也可以单独使用，如图：

![Alt text](assets/split.png)

添加文档后，插件会进行`分片`操作。

> **注意** 拆分方式组合使用时，是有顺序的，从上往下。

> **注意** 如果文档比较大，如图中的文档，三百万字，根据字数`分片`起来会比较久。可以使用根据内容块数量来N等分的方式。
...
直接把光标放在要选择的行内，不做选择，用菜单或者快捷键制作闪卡即可。
