...
这里有三种拆分方式，根据`标题`、内容`块数`量等分N份、根据`字数`，可以同使用，也可以单独使用，如图：

![Alt text](assets/split.png)

拆分方式组合使用时，是有顺序的，从上往下。

举例：如果三种拆分同时启用，根据标题拆分成两份，块数拆分则分别对这两份进行N等分。如果N=4，那么现在我们有2*4=8分。如果根据字数为300再拆分，那么这8份每一份都会按照300字一组进行再拆分。

如果文档比较大，如图中的文档，三百万字，根据字数`分片`起来会比较久。可以使用根据标题以及内容块数量来N等分的组合方式。对于一些文档，导入后并没有标题，单纯N等分需要尝试多次，才能得到好的效果，这时直接按照字数拆分就显得省心。
...
> **提示** 多行选择只能用快捷键来生成闪卡。如果要挖空，另外使用`Alt+D`。
