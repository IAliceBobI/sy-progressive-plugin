...
import { get_siyuan_lnk_md, NewNodeID, siyuan } from "../../sy-tomato-plugin/src/libs/utils";
...
export class WordBuilder {
    ...
    async digest(addCard = false, ai = false) {
        ...
        const wordID = await getWordsDoc(this.bookID, this.boxID, hpath)
        const id = NewNodeID()
        ...
        let targetID = "";
        if (ai) {
            const model = OpenAIClient.getOfficalModel();
            if (model) {
                const config = (await siyuan.getConf())?.conf
                let first = "en_US";
                const second = config.appearance.lang;
                if (first === second) {
                    first = "zh_CN"
                }
                const p = getPrompt(this.rangeText, this.allText, first, second);
                const { targetID: t } = await model(p, innerID);
                targetID = t;
            } else {
